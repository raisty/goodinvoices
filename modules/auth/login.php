...
if (!empty($_POST['user']) && !empty($_POST['pass']) && !empty($_POST['time_val'])) 
{

if (($_POST['time_val']+3)<time())
{
...
////	require_once 'Zend/Auth/Adapter/DbTable.php';
	...
	if ($result->isValid()) {
		...
		*/
        ...
        $errorMessage = $LANG['bad_login'];
	      $login_data = htmlspecialchars($userEmail);
	}

} else {
  $errorMessage = $LANG['fast_login'];
  $login_data = htmlspecialchars($userEmail);
}
...
if($_POST['action'] == 'login' && (empty($_POST['user']) OR empty($_POST['pass'])))
{
        ...
        $errorMessage = $LANG['no_logindata'];
}

$smarty->assign("login_data",$login_data);
$smarty->assign("errorMessage",$errorMessage);
$smarty->assign("time_data",time());
?>
