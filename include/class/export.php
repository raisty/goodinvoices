...
class export
{
	...
	function showData($data)
	{
		...
		switch ($this->format)
		{
			...
			case "pdf":
			{
				$filename = iconv('UTF-8', 'ASCII//TRANSLIT', $this->file_name);
        pdfThis($data, $this->file_location, $filename);
			...
			case "file":
			{
				...
				switch ($this->module)
				{
					case "statement":
					{
						header('Content-Disposition: attachment; filename="statement_'.addslashes($this->id.'.'.$this->file_type).'"');
						break;
					}
					case "payment":
					{
						header('Content-Disposition: attachment; filename="payment_'.addslashes($this->id.'.'.$this->file_type).'"');
						break;
					}
					default:
					{
						$heading=iconv('UTF-8', 'ASCII//TRANSLIT', $preference['pref_inv_heading']);
            header('Content-Disposition: attachment; filename="'.addslashes($heading."_".$this->id.'.'.$this->file_type).'"');
						break;
					}
				}
	...
	function getData()
	{
      ...
      case "payment":
      {
        ...
        $name=Zend_Session::namespaceGet('Zend_Auth');
        $p_logo="templates/invoices/logos/".$name["email"].".png";
         if(file_exists($p_logo)) {
           $po_logo=$siUrl.$p_logo;
         }
         ...
         if(file_exists($p_logo)) {
           $po_logo=$siUrl.$p_logo;
 			  ...
 			  $invoice_number_of_taxes = numberOfTaxesForInvoice($this->id);
				...
				$css = $siUrl."templates/invoices/${template}/";
