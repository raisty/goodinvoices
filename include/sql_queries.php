...
function getTaxTypes() {
	...
	$types=  array(
                                '%' => '%',
                                $locale_info["currency_symbol"] => $locale_info["currency_symbol"],
                                '€' => '€',
                                '$' => '$',
                                '£' => '£',
                                'P' => 'P',
...
function getDefaultNumbering() {
	global $dbh;

	$sql = "SELECT value from ".TB_PREFIX."system_defaults s WHERE ( s.name = 'numbering')";
	$sth = dbQuery($sql) or die(htmlsafe(end($dbh->errorInfo())));
	$entry = $sth->fetch();
	if(!isset($entry['value'])) { $value=0; } else { $value=$entry['value']; }
	return $value;
}

function getDefaultSuppType() {
	global $dbh;

	$sql = "SELECT value from ".TB_PREFIX."system_defaults s WHERE ( s.name = 'suppliers_numbering')";
	$sth = dbQuery($sql) or die(htmlsafe(end($dbh->errorInfo())));
	$entry = $sth->fetch();
	if(!isset($entry['value'])) { $value=0; } else { $value=$entry['value']; }
	return $value;
...
function getInvoiceByIndex($id) {
	global $dbh;
	global $config;
	global $auth_session; 
	
	$sql = "SELECT * FROM ".TB_PREFIX."invoices WHERE index_id =  :id and domain_id =  :domain_id";
	//echo $sql;
...
function insertDefault($name,$value,$extension_name="core") {
	global $auth_session;
	
	$sql = "INSERT into ".TB_PREFIX."system_defaults (id,name,value,domain_id,extension_id) VALUES ('',:name,:value,:domain_id,:extension_id)"; 
	...
	if (dbQuery($sql, ':value', $value, ':name', $name, ':domain_id', $auth_session->domain_id, ':extension_id', $extension)) { 
		return true; 
	}
	return false;
}
...
function updateInvoice($invoice_id) {
    ...
    $old_biller_id = $current_invoice['biller_id'];
    $new_biller_id = $_POST['biller_id'];

//	$logger->log('Curent Index Group: '.$description, Zend_Log::INFO);
//	$logger->log('Description: '.$description, Zend_Log::INFO);

    $supp_type = getDefaultSuppType();

    if ($old_biller_id != $new_biller_id && $supp_type==1)
    {
        $index_id = index::increment('invoice',$new_pref_group['index_group'],$new_biller_id);
        $old_index_id = index::rewind('invoice',$current_pref_group['index_group'],$old_biller_id);
    }
    else
    {
        if ($current_pref_group['index_group'] != $new_pref_group['index_group'])
        {
            ...
            $old_index_id = index::rewind('invoice',$current_pref_group['index_group']);
        }
    }
