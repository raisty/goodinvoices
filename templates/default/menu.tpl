<div class="top_menu">
  <div class="txt_left">
    <div style="font-family: times, Times New Roman, times-roman, georgia, serif; font-size: 18px; font-style: italic;">{php}echo htmlspecialchars($_SERVER['SERVER_NAME']);{/php}:&thinsp;{$design}</div>
  </div>
	<div class="txt_right">
    {$LANG.hello} <a href="index.php?module=user&amp;view=manage">{$smarty.session.Zend_Auth.email|htmlsafe}</a> | <a href="https://bitbucket.org/maxsamael/simpleinvoices-fork/wiki/Home" target="_blank">{$LANG.help}</a>
		| <a href="index.php?module=system_defaults&amp;view=edit&amp;submit=language">{$LANG.language}: {$country}{if $curr_sign!=""}<small>[{$curr_sign}]</small>{/if}</a>
